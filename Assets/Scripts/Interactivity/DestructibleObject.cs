...
public class DesturctibleObject : MonoBehaviour, IRaycastHittable
{
    ...
    private void Explode()
    {
        ...
        foreach (Rigidbody fraction in _objectFractions)
        {
            ...
            fraction.AddForce((fraction.transform.position - this.transform.position) * Random.Range(20f, 50f), ForceMode.Impulse);
        }

        GameObject dropItem = Instantiate(_sceneItemPrefab, transform.position, transform.rotation);
